...
tanent_id = dbutils.secrets.get(scope="adbtesthudsonstorage-scope", key="adb-test-hudson-databricks-tenantid")
client_secret = dbutils.secrets.get(scope="adbtesthudsonstorage-scope", key="adb-test-hudson-databricks-client-secret")

print(dbutils.secrets.listScopes())
print(dbutils.secrets.list('adbtesthudsonstorage-scope'))

spark.conf.set("fs.azure.account.auth.type.adbtesthudsonstorage.dfs.core.windows.net", "OAuth")
spark.conf.set("fs.azure.account.oauth.provider.type.adbtesthudsonstorage.dfs.core.windows.net", "org.apache.hadoop.fs.azurebfs.oauth2.ClientCredsTokenProvider")
spark.conf.set("fs.azure.account.oauth2.client.id.adbtesthudsonstorage.dfs.core.windows.net", client_id)
spark.conf.set("fs.azure.account.oauth2.client.secret.adbtesthudsonstorage.dfs.core.windows.net", client_secret)
spark.conf.set("fs.azure.account.oauth2.client.endpoint.adbtesthudsonstorage.dfs.core.windows.net", f"https://login.microsoftonline.com/{tanent_id}/oauth2/token")
